...
import { getStudentErrorSummary } from "./kaled-error-memory";
...
export async function runAlertEngine(tenantId: string): Promise<number> {
  ...
  const instructor = await prisma.user.findFirst({
    where: { tenantId, platformRole: { in: ["ACADEMY_TEACHER", "ACADEMY_ADMIN"] } },
    select: { id: true },
  });
...
async function detectStudentIssues(params: {
  ...
  const [errorSummary, dailyUsage, pendingDeliverables] = await Promise.all([
    ...
    prisma.academyDeliverableSubmission.count({
      where: {
        ...
        submittedAt: null,
        updatedAt: {
          lte: new Date(Date.now() - THRESHOLDS.DELIVERABLE_OVERDUE_DAYS * 86400000),
        },
      },
    }),
  ]);
  ...
  if (snapshot.kaledInteractions > THRESHOLDS.KALED_NO_PROGRESS &&
