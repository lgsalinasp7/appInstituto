...
// Ubicación sugerida: /apps/kaledacademy/services/
//
// Convención KaledSoft:
//   - Todos los servicios reciben tenantId en el primer arg
//   - Errores se lanzan con throw new Error() descriptivo
//   - El caller (API Route / Server Action) hace el try/catch
//   - Nunca exponer detalles internos al cliente
// ============================================================

import { prisma } from "@/lib/prisma";
import { CRALPhase, DeliverableStatus, SaasProjectStatus } from "@prisma/client";
...
export const courseService = {
  /**
   * Obtener el curso del bootcamp del tenant con todos sus módulos y lecciones.
   * Un tenant tipo "kaledacademy" tiene exactamente 1 curso activo (el bootcamp).
   */
  ...
  /**
   * Obtener una sola lección con todo su contenido educativo y
   * el progreso del usuario si se provee userId.
   */
  async getLessonWithProgress(lessonId: string, tenantId: string, userId?: string) {
    ...
    const [progress, quizResults, cralCompletions, deliverySubmission] = await Promise.all([
      ...
      prisma.academyCRALCompletion.findMany({
        where: { userId, challengeId: { in: lesson.cralChallenges.map(c => c.id) } }
      }),
      lesson.deliverables[0]
        ? prisma.academyDeliverableSubmission.findUnique({
            where: { userId_deliverableId: { userId, deliverableId: lesson.deliverables[0].id } }
          })
        : null
    ]);
  ...
  /**
   * Lista resumida de módulos para el sidebar de navegación.
   * Incluye conteo de lecciones completadas por el usuario.
   */
...
export const progressService = {
  /**
   * Marcar una lección como completada y recalcular el snapshot del estudiante.
   * Verifica que el usuario esté matriculado en el curso.
   */
  async markLessonCompleted(
    ...
    if (!enrollment) throw new Error("El usuario no está matriculado en este curso");
    ...
    // Recalcular progreso global del enrollment
    await this._recalculateEnrollmentProgress(userId, lesson.module.courseId, tenantId);

    return progress;
  },

  /**
   * Actualizar el progreso de video (sin marcar como completado).
   */
  async updateVideoProgress(userId: string, lessonId: string, videoProgress: number) {
    return prisma.academyStudentProgress.upsert({
      where: { userId_lessonId: { userId, lessonId } },
      create: { userId, lessonId, videoProgress, videoWatchedAt: new Date() },
      update: { videoProgress, videoWatchedAt: new Date(), updatedAt: new Date() }
    });
  },

  /**
   * Recalcular y persistir el progreso total del enrollment.
   * Se llama internamente después de cada acción de progreso.
   */
  async _recalculateEnrollmentProgress(userId: string, courseId: string, tenantId: string) {
    ...
    // Actualizar snapshot del estudiante
  ...
  /**
   * Refrescar el snapshot agregado del estudiante para el dashboard.
   */
  async _refreshStudentSnapshot(userId: string, courseId: string, tenantId: string) {
    const enrollment = await prisma.academyEnrollment.findFirst({
    ...
    const [
      ...
      prisma.academyStudentProgress.count({ where: { userId, completed: true } }),
      prisma.academyLesson.count({ where: { module: { courseId }, isActive: true } }),
      ...
      prisma.academyDeliverableSubmission.count({ where: { userId, status: "APROBADO" } }),
    ...
    const cralMap = cralData.reduce((acc, row) => {
    ...
    await prisma.academyStudentSnapshot.upsert({
      ...
      create: {
        userId, cohortId: enrollment.cohortId, enrollmentId: enrollment.id,
        overallProgress: enrollment.progress,
        lessonsCompleted, lessonsTotal,
        quizzesPassed, quizzesTotal,
        deliverablesSubmitted, deliverablesApproved,
        cralCompleted: cralMap,
        kaledInteractions, tenantId,
...
export const quizService = {
  /**
   * Registrar la respuesta de un estudiante a un quiz.
   * Devuelve si fue correcta y el feedback de Kaled.
   */
  async submitAnswer(userId: string, quizId: string, selectedOptionId: string) {
...
export const cralService = {
  /**
   * Marcar una fase CRAL como completada por el estudiante.
   */
  ...
  /**
   * Obtener el estado CRAL del estudiante en una lección.
   */
...
export const deliverableService = {
  ...
  /**
   * Revisar y calificar un entregable (instructor).
   */
  async review(
    submissionId: string,
    reviewedById: string,
  ...
  /**
   * Listar entregables pendientes de revisión para el instructor.
   */
  async getPendingReviews(cohortId: string, tenantId: string) {
    return prisma.academyDeliverableSubmission.findMany({
      where: {
        status: "ENTREGADO",
        deliverable: { lesson: { module: { course: { cohorts: { some: { id: cohortId } } } } } }
      },
      include: {
...
export const cohortService = {
  /**
   * Obtener la cohorte activa del tenant con sus estudiantes y progreso.
   */
  ...
  /**
   * Matricular un estudiante en la cohorte activa del tenant.
   * Crea el User (si no existe), el Enrollment y el Snapshot inicial.
   */
  async enrollStudent(
    tenantId: string,
    courseId: string,
    ...
    if (existingEnrollment) throw new Error("El estudiante ya está matriculado");
    ...
    await prisma.academyStudentSnapshot.create({
      data: {
        userId: studentData.userId,
        cohortId, enrollmentId: enrollment.id,
        overallProgress: 0, lessonsCompleted: 0, lessonsTotal: 0,
        quizzesPassed: 0, quizzesTotal: 0,
        deliverablesSubmitted: 0, deliverablesApproved: 0,
        kaledInteractions: 0, tenantId
      }
    });

    // Actualizar contador de estudiantes en la cohorte
  ...
  /**
   * Dashboard del instructor: ranking de estudiantes por progreso.
   */
...
export const saasProjectService = {
  /**
   * Crear o actualizar el proyecto SaaS del estudiante.
   */
  async upsertProject(
    userId: string,
    cohortId: string,
  ...
  /**
   * Registrar actualización semanal del proyecto.
   */
  async addWeeklyUpdate(
    projectId: string,
    weekNumber: number,
    data: { githubUrl?: string; deployUrl?: string; summary: string; cralDone?: Record<string, boolean> }
  ) {
  ...
  /**
   * Registrar resultado del Demo Day.
   */
  async recordDemoDayResult(
    ...
    const total =
      (scores.functionality + scores.architecture + scores.code +
       scores.realUsers + scores.aiCriteria) / 5;

    const result = await prisma.academyDemoDayResult.upsert({
      where: { projectId },
      create: {
        projectId, cohortId, tenantId,
...
export const kaledAIService = {
  /**
   * Crear una sesión de chat con Kaled en el contexto de una lección.
   * Reutiliza AiConversation existente de KaledSoft.
   */
  async startSession(
    userId: string,
    tenantId: string,
    ...
    await prisma.academyKaledSession.create({
      data: {
        ...
        sessionType: (context.sessionType as any) ?? "LESSON_SUPPORT"
      }
    });
  ...
  /**
   * Construir el system prompt de Kaled basado en el contexto.
   * El system prompt varía según la lección activa del estudiante.
   */
...
- Nunca inventes información sobre el temario — si no sabes algo del bootcamp, di "Consulta el temario en la plataforma".`;
  ...
  /**
   * Construir el contexto de la lección para el mensaje del sistema.
   * (usado al inicio de cada sesión con una lección específica)
   */
...
export const badgeService = {
  /**
   * Verificar y otorgar badges después de una acción relevante.
   * Llamar después de markLessonCompleted, submit deliverable, etc.
   */
...
export const analyticsService = {
  /**
   * Dashboard de métricas para el instructor/admin de la academia.
   */
  async getCohortAnalytics(cohortId: string, tenantId: string) {
    const [
      ...
      prisma.academyEnrollment.count({ where: { cohortId, status: "ACTIVE" } }),

      prisma.academyStudentSnapshot.count({
        where: { cohortId, lastActivityAt: { gte: new Date(Date.now() - 7 * 86400000) } }
      }),
      ...
      prisma.academyDeliverableSubmission.groupBy({
        ...
        where: {
          deliverable: {
            lesson: { module: { course: { cohorts: { some: { id: cohortId } } } } }
          }
        },
        _count: { status: true }
      }),

      prisma.academyQuizResult.aggregate({
        where: {
          quiz: {
            lesson: { module: { course: { cohorts: { some: { id: cohortId } } } } }
          }
        ...
        _count: { id: true },
        _sum: { isCorrect: true } // truco: boolean se suma como 1/0 en PostgreSQL
      }),

      // Las 5 lecciones con más tiempo invertido
    ...
    return {
      ...
      quizAccuracy: quizStats._count.id > 0
        ? Math.round(((quizStats._sum as any).isCorrect / quizStats._count.id) * 100)
        : 0,
