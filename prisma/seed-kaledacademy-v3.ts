...
async function crearSesion(
  ...
  if (lessonId) {
    await actualizarSesion(prisma, lessonId, tenantId, s);
    return null as any;
  }
...
// ── Helper para ACTUALIZAR sesión existente (solo contenido) ─
async function actualizarSesion(
  ...
  await prisma.academyLessonMeta.upsert({
    ...
    update: {
      ...
      dayOfWeek: s.dia as any,
      videoUrl: s.video?.url ?? undefined,
      videoTitle: s.video?.titulo ?? undefined,
      kaledIntro: s.kaledIntro,
      analogyText: s.analogia,
      concepts: s.conceptos as any,
    },
...
const CONTENT_ONLY = process.env.CONTENT_ONLY === "1";

async function mainContentOnly() {
  console.log("🚀 Seed V3 — solo actualización de contenido (sin usuarios)...\n");
  ...
  for (let i = 0; i < 4; i++) {
    ...
    const modWithLessons = { id: mod.id, lessons: mod.lessons };
    if (i === 0) await seedModulo1(prisma, course.id, tenant.id, modWithLessons);
    else if (i === 1) await seedModulo2(prisma, course.id, tenant.id, modWithLessons);
    else if (i === 2) await seedModulo3(prisma, course.id, tenant.id, modWithLessons);
    else if (i === 3) await seedModulo4(prisma, course.id, tenant.id, modWithLessons);
  ...
  for (const mem of memorias) {
    await prisma.agentMemory.create({ data: { agentType: "KALED", tenantId: tenant.id, ...mem, metadata: { source: "seed_v3" } } });
  }

  console.log(`\n✅ Contenido V3 actualizado. ${total} lecciones.`);
}

// ── MAIN ────────────────────────────────────────────────────
async function main() {
  if (CONTENT_ONLY) {
    await mainContentOnly();
    return;
  }
    ...
    create: {
      ...
      branding: {
        create: {
          ...
          footerText: "KaledSoft Technologies · Colombia",
        },
      },
    },
...
async function seedModulo1(
  prisma: PrismaClient,
  courseId: string,
  tenantId: string,
  existingModule?: { id: string; lessons: { id: string }[] }
) {
  ...
  await crearSesion(prisma, mod.id, tenantId, {
    ...
    video: { url: "https://www.youtube.com/watch?v=GkA5WOeLWbM", titulo: "¿Cómo funciona internet? — Explicación en español" },
