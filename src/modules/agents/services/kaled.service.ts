...
- NUNCA inventes cifras, CPL, gastos, leads, conversiones ni fuentes de tráfico
- Si te piden métricas exactas y no cuentas con datos verificados en el contexto/herramientas, responde exactamente:
  "No tengo datos verificados para calcular ese indicador en este momento."
- Cuando no haya datos verificables, ofrece solicitar o consultar el dashboard en lugar de estimar
`;
  }
  ...
  static async chat(
    ...
    if (!hasGroqKey && !hasAnthropicKey && !hasGoogleKey) {
      throw new Error(
        'No hay proveedor IA configurado para Kaled. Configure GROQ_API_KEY, GOOGLE_GENERATIVE_AI_API_KEY o ANTHROPIC_API_KEY.'
      );
    }
...
`;
    ...
    if (hasGroqKey) {
      try {
        const result = await streamText({
          ...
          // Groq en este flujo se usa sin tools por estabilidad.
  ...
  static async generateBriefing(
    ...
    if (!hasGroqKey && !hasAnthropicKey && !hasGoogleKey) {
      throw new Error(
        'No hay proveedor IA configurado para generar briefings. Configure GROQ_API_KEY, GOOGLE_GENERATIVE_AI_API_KEY o ANTHROPIC_API_KEY.'
      );
    }
