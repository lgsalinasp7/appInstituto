...
export function ReceiptConfirmationModal({ isOpen, onClose, data }: ReceiptConfirmationModalProps) {
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4 animate-fade-in-up">
      <div className={cn(
        "rounded-2xl shadow-2xl w-full max-w-sm max-h-[80vh] flex flex-col overflow-hidden border transition-colors",
        isDark ? "bg-slate-900 border-white/10" : "bg-white border-gray-200"
      )}>
        {/* Header con icono de éxito */}
        <div className={cn(
          "py-3 px-4 text-white text-center relative shrink-0",
          isDark ? "bg-gradient-to-r from-emerald-600 to-emerald-700" : "bg-gradient-to-r from-emerald-500 to-emerald-600"
        )}>
          <button
            onClick={onClose}
        ...
        <div className="p-3 space-y-2 overflow-y-auto custom-scrollbar flex-1">
          ...
          <div className={cn(
            ...
            )}>{data.payment.receiptNumber}</p>
            <p className="text-[10px] text-gray-400 leading-none mt-0.5">{formatDate(data.payment.paymentDate)}</p>
          </div>

          {/* Datos del estudiante */}
          <div className={cn(
            "space-y-1 text-xs border-y py-2",
            isDark ? "border-slate-800" : "border-gray-100"
          )}>
            <div className="flex justify-between items-center">
              <span className="text-gray-500">Estudiante:</span>
              <span className={cn("font-bold text-right max-w-[170px] truncate", isDark ? "text-slate-300" : "text-primary")}>{data.student.fullName}</span>
            </div>
            <div className="flex justify-between items-center">
              <span className="text-gray-500">Documento:</span>
              <span className={cn("font-medium", isDark ? "text-slate-400" : "text-gray-700")}>{data.student.documentType} {data.student.documentNumber}</span>
            </div>
            <div className="flex justify-between items-center">
          ...
          <div className={cn(
            ...
            <div className="flex justify-between items-center mb-1">
              <span className={cn("text-[10px]", isDark ? "text-slate-400" : "text-gray-600")}>Concepto:</span>
              <span className={cn("font-bold text-[10px]", isDark ? "text-emerald-400" : "text-emerald-700")}>Pago de Matrícula</span>
            </div>
            <div className="flex justify-between items-center mb-1">
              <span className={cn("text-[10px]", isDark ? "text-slate-400" : "text-gray-600")}>Método:</span>
              <span className={cn("font-medium text-[10px] flex items-center gap-1", isDark ? "text-slate-300" : "text-gray-700")}>
                <CreditCard size={10} />
                {formatMethod(data.payment.method)}
              </span>
            </div>
            <div className={cn(
              "flex justify-between items-center pt-1 border-t",
              isDark ? "border-emerald-500/20" : "border-emerald-200"
            )}>
              <span className={cn("text-xs font-bold", isDark ? "text-slate-300" : "text-gray-700")}>TOTAL:</span>
          ...
          <div className={cn(
            ...
            <div className={cn("flex justify-between text-[10px]", isDark ? "text-slate-300" : "text-gray-700")}>
              <span className="text-gray-500">Módulo {data.commitment.moduleNumber}:</span>
        ...
        {/* Botones de acción */}
        <div className={cn("p-3 pt-0 shrink-0 space-y-2", isDark ? "bg-slate-900" : "bg-white")}>
          ...
          <button
            onClick={onClose}
            className={cn(
              "w-full py-2 rounded-lg font-bold transition-all text-xs h-8",
              isDark ? "bg-slate-800 text-slate-400 hover:bg-slate-700" : "bg-gray-100 text-gray-700 hover:bg-gray-200"
            )}
          >
            Cerrar
          </button>
