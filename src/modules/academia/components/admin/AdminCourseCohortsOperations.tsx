...
interface CourseCohortsData {
  id: string;
  title: string;
  cohorts: Cohort[];
}

export function AdminCourseCohortsOperations({ courseId }: { courseId: string }) {
  const router = useRouter();
  const searchParams = useSearchParams();
  const openedNewFromQuery = useRef(false);

  useEffect(() => {
    openedNewFromQuery.current = false;
  }, [courseId]);

  const [course, setCourse] = useState<CourseCohortsData | null>(null);
  ...
  useEffect(() => {
    if (!course || loading || openedNewFromQuery.current) return;
    if (searchParams.get("new") !== "1") return;
    openedNewFromQuery.current = true;
    setCohortModal({ open: true });
    router.replace(`/academia/admin/cohorts?courseId=${encodeURIComponent(courseId)}`, { scroll: false });
  }, [course, loading, searchParams, courseId, router]);
  ...
  const handleAssignTeacher = async () => {
    ...
    if (!c || !raw) return;
    const payload = raw.includes("@") ? { teacherEmail: raw } : { teacherUserId: raw };
    try {
  ...
  if (loading || !course) {
    return (
      <div className="academy-card-dark p-8">
        <p className="text-slate-400">Cargando cohortes del curso…</p>
      </div>
    );
  }
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
        <div>
          ...
          <h2 className="text-2xl font-black tracking-tight text-white font-display">Cohortes del curso</h2>
          <p className="text-slate-400 mt-1 text-sm">
            <span className="text-slate-300 font-medium">{course.title}</span>
            {" · "}
            Alta, edición, profesores, mover o unir matrículas.
          </p>
        </div>
        <div className="flex flex-wrap items-center gap-2 shrink-0">
          ...
          <Button
            ...
            className="rounded-xl border-white/[0.10] bg-white/[0.04] text-slate-200 shadow-none hover:bg-white/[0.08] hover:text-white hover:border-white/[0.14]"
          >
            <Link href={`/academia/admin/courses/${courseId}`}>Contenido del curso</Link>
          </Button>
        </div>
      </div>

      <div className="academy-card-dark p-6 space-y-4">
        <h3 className="text-lg font-bold text-white font-display">Cohortes de este programa</h3>
        <p className="text-xs text-slate-500 -mt-2">
          Cada tarjeta alinea el contenido del cohorte con el bloque de estado y acciones, sin líneas sueltas.
        </p>
        <div className="space-y-2">
          {course.cohorts.map((c) => (
            <div
              key={c.id}
              className={cn(
                "grid grid-cols-1 sm:grid-cols-[minmax(0,1fr)_auto] gap-4 sm:gap-0 sm:items-center",
                "rounded-xl border border-white/[0.06] bg-white/[0.02] px-4 py-4"
              )}
            >
              <div className="flex gap-3 min-w-0 sm:pr-5">
                <CalendarDays className="w-4 h-4 text-slate-500 shrink-0 mt-0.5" aria-hidden />
                <div className="min-w-0 space-y-2">
                  <div className="space-y-0.5">
                    <p className="font-semibold text-white leading-snug">{c.name}</p>
                  ...
                  <div className="flex flex-wrap items-center gap-x-2 gap-y-1 text-xs">
                    <span className="text-[11px] text-slate-600">Ir a:</span>
                    <Link
              ...
              <div
                className={cn(
                  ...
                  "sm:w-auto sm:flex-col sm:items-end sm:justify-center sm:gap-2.5 sm:self-center",
                  "border-t border-white/[0.06] pt-4 sm:border-t-0 sm:border-l sm:pt-0 sm:pl-5"
                )}
              >
                <span className="text-xs px-2.5 py-1 rounded-md bg-slate-500/20 text-slate-200 font-medium border border-white/[0.06]">
                  {c.status}
                </span>
                <div
                  className="flex flex-wrap items-center justify-end gap-1.5"
                  role="toolbar"
                  aria-label="Acciones del cohorte"
                >
                  {(
                    [
                      {
                      ...
                      },
                      {
                        ...
                        icon: ArrowRightLeft,
                        className: "text-slate-400 hover:text-cyan-300 hover:bg-white/5",
                      },
                      {
                        key: "merge",
                        ...
                        icon: GitMerge,
                        className: "text-slate-400 hover:text-amber-300 hover:bg-white/5",
                      },
                      {
                        key: "edit",
                        title: "Editar cohorte",
                        onClick: () => setCohortModal({ open: true, editing: c }),
                        icon: Pencil,
                        className: "text-slate-400 hover:text-white hover:bg-white/5",
                      },
                      {
                  ...
                  ).map(({ key, title, onClick, icon: Icon, className }) => (
                    <button
                      key={key}
                      type="button"
                      onClick={onClick}
        ...
        >
          <DialogHeader className="shrink-0 px-6 pt-6 pr-14 text-left">
            <DialogTitle className="text-white">
              {cohortModal.editing ? "Editar cohorte" : "Nuevo cohorte"}
            </DialogTitle>
          </DialogHeader>
          <form onSubmit={(e) => void handleSaveCohort(e)} className="flex min-h-0 flex-1 flex-col">
            <div className="min-h-0 flex-1 space-y-4 overflow-y-auto overflow-x-hidden px-6 py-2 overscroll-contain">
              ...
              <div>
                ...
                <p className="text-[11px] text-slate-500 mt-1">
                  Académico: orientación típica ~4 meses (fechas manuales). Promocional: mismo curso, acceso
                  limitado en tiempo; respeta trialAllowedLessonId en la matrícula.
                </p>
              </div>
              <div>
              ...
              <div>
                ...
                />
                <p className="text-[11px] text-slate-500 mt-1">
                  Matrículas activas + invitaciones pendientes no pueden superar este número. Ajusta si necesitas
                  más cupo.
                </p>
              </div>
              <div>
      ...
      <Dialog open={moveModal.open} onOpenChange={(open) => setMoveModal({ open, source: open ? moveModal.source : null })}>
        <DialogContent className="academy-card-dark border-white/10 bg-slate-900">
          ...
          <p className="text-sm text-slate-400">
            Origen: <span className="text-white">{moveModal.source?.name}</span>. Selecciona cohorte destino (mismo
            curso).
          </p>
          <TargetCohortSelect
      ...
      <Dialog open={mergeModal.open} onOpenChange={(open) => setMergeModal({ open, source: open ? mergeModal.source : null })}>
        <DialogContent className="academy-card-dark border-white/10 bg-slate-900">
          ...
          <p className="text-sm text-slate-400">
            Origen: <span className="text-white">{mergeModal.source?.name}</span>. Todas las matrículas pasan al
            destino y el origen se marca cancelado.
          </p>
          <TargetCohortSelect
      ...
      >
        <DialogContent className="academy-card-dark border-white/10 bg-slate-900 max-w-md">
          ...
          <div className="space-y-2 max-h-40 overflow-y-auto">
            ...
            {teacherList.length === 0 && <p className="text-slate-500 text-sm">Ningún profesor asignado.</p>}
          </div>
  ...
  return (
    <div className="space-y-4">
      ...
      <DialogFooter>
        ...
        <Button type="button" className="bg-cyan-600" disabled={!val} onClick={() => onConfirm(val)}>
          Confirmar
        </Button>
      </DialogFooter>
    </div>
