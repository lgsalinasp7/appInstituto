      ...
      try {
        if (!context?.params) {
          return NextResponse.json(
            { success: false, error: 'Parámetros de ruta no disponibles' },
            { status: 500 }
          );
        }
        ...
        console.error('[PATCH tenant user]', err);
        return NextResponse.json(
          { success: false, error: err instanceof Error ? err.message : 'Error al actualizar usuario' },
          { status: 500 }
        );
      }
    }
