...
| # | Tarea | Agente | Estado |
|---|---|---|---|
| 2.1 | Migrar 63 API routes con `console.log/error` directo a `logApiStart/Success/Error` | Dev | PARCIAL PR `#42` merged — 18/63 routes P0+P1 migradas (webhooks/resend, auth/*, public/aplicar, admin/kaled-leads, admin/email-templates, etc). 45 routes pendientes para 2.1b. `chat/stream/route.ts` skipped (streaming SSE requiere refactor mayor). |
...
| 2.6 | E2E: anadir 8 specs criticas | QA | PARCIAL PR `#40` merged — 4/8 specs creadas (enrollment, payment, lavadero-kanban, cartera-commitment) + 8 tests con `test.skip` (requiere seed test). Pendientes: leccion, leaderboard, invitation, recaudo. |
...
| 2.8 | Limpiar violaciones residuales `no-explicit-any` | Dev | PARCIAL PR `#43` merged — 144 → 107 violaciones (-25.7%, 21 archivos). Patrones: `unknown`+type guards, enums Prisma, eslint-disable justificado en recharts/AI SDK v6. Pendiente: ~107 restantes (cascadean en CampaignsClient, LeadsClient, UsersClient, empresas que usan Prisma sin extender). |
...
| 2.10 | Configurar CI/CD: lint + type-check + test obligatorios pre-merge | Infra | PARCIAL — tsc + test bloqueantes ya (1.9). Lint avanzo a 107 violaciones (de 191). Hacer bloqueante cuando llegue a 0 (tras 2.1b + iteraciones 2.8). |
